...
showAll(): void {
  ...
  private getExpiryBadgeLabel(fecha: string | undefined): string {
    if (!fecha) return 'Sin fecha';
    if (this.isExpired(fecha)) return 'Vencido';
    if (this.isExpiringSoon(fecha)) return 'Próximo a vencer';
    return 'Vigente';
  }
  ...
  private getInspectionStatusBadgeHtml(status: string | undefined): string {
    ...
    const badgeClass =
      normalized === 'aprobada' || normalized === 'aprbada'
  ...
  private buildSearchResultHtml(
    inspection: Inspection,
    source: Inspection[],
    isLatestForPlate: boolean
  ): string {
  ...
  private buildGroupedSearchResultsHtml(matches: Inspection[], source: Inspection[]): string {
    ...
    return [...grouped.entries()]
      .sort(([, inspectionsA], [, inspectionsB]) => {
        const latestA = [...inspectionsA].sort(
          (a, b) => this.getInspectionDateTime(b) - this.getInspectionDateTime(a)
        )[0];
        const latestB = [...inspectionsB].sort(
          (a, b) => this.getInspectionDateTime(b) - this.getInspectionDateTime(a)
        )[0];
        ...
        return `
          <section class="search-result-group">
            ...
            ${sortedInspections.map((inspection) =>
              this.buildSearchResultHtml(inspection, source, !!latest && inspection.id === latest.id)
            ).join('')}
          </section>
        `;
      })
